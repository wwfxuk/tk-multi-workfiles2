...
class FileFinder(QtCore.QObject):
    ...
    def _find_work_files(self, context, work_template, version_compare_ignore_fields):
        ...
        # if "User" not in skip_fields:
        #     skip_fields += ["User"]

        # find paths:
        import traceback
        self._app.logger.debug('Finding work files..............')
        self._app.logger.debug('work_template: %s', work_template)
        self._app.logger.debug('  work_fields: %s', work_fields)
        self._app.logger.debug('  skip_fields: %s', skip_fields)
        self._app.logger.debug(''.join(traceback.format_stack()))
...
class AsyncFileFinder(FileFinder):
    ...
    def _begin_search_for_work_files(self, search, work_area):
        ...
        """
        import traceback
        self._app.logger.debug('_begin_search_for_work_files.............')
        self._app.logger.debug('search.users: %s', search.users)
        self._app.logger.debug('   work_area: %s', work_area)


        # So basically, no files will be searched for if no users are selected
        # Surely we would want to search ALL users instead?
        # i.e. add User to version_compare_ignore_fields include
        if not search.users:
            search.users.append(None)
            if 'User' not in work_area.version_compare_ignore_fields:
                work_area.version_compare_ignore_fields.append('User')
        ...
        for user in search.users:
            ...
            self._app.logger.debug('            user_id: %s', user_id)
            self._app.logger.debug(' (calc) work_fields: %s', user_work_area.context.as_template_fields(user_work_area.work_template, validate=True))
